# خطة مرحلة v0.4 — UI احترافي + موافقات حكومية أعمق + تخزين إنتاجي

## المسارات الثلاثة المطلوبة

### 1) UI احترافي
- اعتماد App Shell كامل + صفحات متخصصة بدل شاشة تشغيل واحدة
- جداول احترافية مع فلاتر/فرز/بحث/صفحات
- نماذج تحقق واضحة + حالات تحميل/خطأ/نجاح + دعم RTL متقن
- لوحات Users / Approvals / Attachments / Projects / Content

### 2) موافقات حكومية أعمق
- حالات قرار متعددة (اعتماد/رفض/طلب تعديل/تصعيد)
- SLA فعلي + تنبيهات + escalations حسب الجهة ونوع المحتوى
- مسار تدقيق كامل audit trail قابل للتصدير
- مصفوفة صلاحيات RBAC أدق + مستويات اعتماد متعددة

### 3) تخزين إنتاجي
- استبدال التخزين المحلي بموفر Object Storage (S3 compatible)
- روابط موقعة Signed URLs للرفع/التحميل
- سياسات نوع/حجم ملفات + مضاد تكرار + فحص مبدئي
- فصل metadata (DB) عن binary storage + سياسات retention

## مخرجات v0.4 المقترحة
1. واجهة إدارة احترافية كاملة
2. صندوق موافقات حكومي مع Timeline وSLA
3. تخزين إنتاجي Signed URLs + سياسات رفع
4. مؤشرات تشغيل أساسية للمراجعات والملفات

## مرجع تفصيلي إضافي للواجهة
