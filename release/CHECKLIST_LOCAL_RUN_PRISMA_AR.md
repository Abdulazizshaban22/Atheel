# Checklist تشغيل محلي + أوامر Prisma خطوة بخطوة — أَثِيل v0.3

هذا الملف يلبي طلب التشغيل المحلي مع تركيز خاص على أوامر Prisma خطوة بخطوة.

## أوامر Prisma الأساسية (تسلسل مقترح)
1. إنشاء/تحديث عميل Prisma
```bash
cd packages/db
pnpm prisma generate
```

2. تطبيق الهجرات على بيئة التطوير
```bash
pnpm prisma migrate dev
```

3. تعبئة البيانات الأولية
```bash
pnpm prisma db seed
```

4. (اختياري) إعادة ضبط قاعدة التطوير ثم إعادة التهيئة
```bash
pnpm prisma migrate reset
```

5. لبيئة أقرب للإنتاج بعد بناء الحزم
```bash
pnpm prisma migrate deploy
```

## Checklist التشغيل المحلي الكامل
