...
export type AgentRole = 'researcher' | 'curator' | 'program_designer' | 'ops_manager' | 'risk_auditor';

export type AgentActionNodeData = {
  label: string;
  role: AgentRole;
  hint?: string;
};

const ROLE_LABEL_AR: Record<AgentRole, string> = {
  researcher: 'باحث ثقافي',
  curator: 'قيّم',
  program_designer: 'مصمم برنامج',
  ops_manager: 'مدير تشغيل',
  risk_auditor: 'مدقق مخاطر',
};

export function AgentActionNode(props: NodeProps<AgentActionNodeData>) {
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 11, color: '#64748b' }}>{data.hint || ROLE_LABEL_AR[data.role]}</div>
        </div>
      ...
      <div style={{ marginTop: 8, fontSize: 11, color: '#334155' }}>
        الدور: <b>{ROLE_LABEL_AR[data.role]}</b>
      </div>
