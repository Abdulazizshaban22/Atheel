...
export function JoinGateNode(props: NodeProps<JoinGateNodeData>) {
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 11, color: '#64748b' }}>{data.hint || 'دمج دلالي'}</div>
          {data.mode === 'OR' && data.cancelOthers ? (
            <div style={{ marginTop: 6, fontSize: 11, color: '#b45309' }}>يلغي الفروع الأخرى عند أول وصول</div>
          ) : null}
        </div>
